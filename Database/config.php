...
// this is for main_aadmin login
// if(isset($_POST["main_admin_log"]))
// {

//   $res=mysqli_query($db,"SELECT * from mainadmin WHERE  username_main='$_POST[username_main]' && password_main='$_POST[password_main]'");
//     if($row=mysqli_fetch_array($res)){
...
// for username validation
  ...
  // check email & username already exist or not
  $user_check_query = "SELECT * FROM jud_user WHERE username='$username' OR email='$email' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($result);
  ...
  if (count($errors) == 0) {
    ...
    //Insert data in database
  	$query = "INSERT INTO jud_user (username, email, password,address,phone) 
  			  VALUES('$username', '$email', '$password', '$address', '$phone')";
  	mysqli_query($db, $query);
    $_SESSION['username'] = $username;
    //if all operation success
...
// if everything okey then
  if (count($errors) == 0) {
  	$password = md5($password);
  	$query = "SELECT * FROM jud_user WHERE username='$username' AND password='$password'";
    $results = mysqli_query($db, $query);
...
if(isset($_POST["admin"])){
 ...
 mysqli_query($db,"INSERT into product VALUES('','$_POST[pnm]',$_POST[ppr],'$_POST[ppr]','$dst','$_POST[pcat]','$_POST[pdes]') ");
 
 }
 //case
 if(isset($_POST["case_admin"])){
  $v1=rand(1000,5555);
  $v2=rand(1000,5555);
 $v3=$v1.$v2;
 ...
 mysqli_query($db,"INSERT into product_case VALUES('','$_POST[pnm]',$_POST[ppr],'$_POST[ppr]','$dst','$_POST[pcat]','$_POST[pdes]') ");
 
 }

//  inves
 if(isset($_POST["inv_admin"])){
  $v1=rand(1000,5555);
  $v2=rand(1000,5555);
 $v3=$v1.$v2;
 ...
 mysqli_query($db,"INSERT into product_inv VALUES('','$_POST[pnm]',$_POST[ppr],'$_POST[ppr]','$dst','$_POST[pcat]','$_POST[pdes]') ");
 
 }

//  if(isset($_POST["done"])){
//   mysqli_query($db,"INSERT into judgeview VALUES('',$_POST[qus],'$_POST[ans]') ");
